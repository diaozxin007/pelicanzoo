// The papers a fed pelican gets once the critic has been round.
//
// This is string templating with no imports beyond bandOf, which is the whole
// point: the same function renders the committed /badge/<id>.svg at build time
// and the provisional badge the worker hands a feeder on the receipt page
// seconds after they submit. Two renderers would drift, and the drift would
// show up as a badge that changed the day a pelican was let into the zoo.
//
...
/** @param {{ model: string, by?: string|null, svg: string,
 *            score: number, review: string, critic?: string|null }} s */
export function renderBadge(s) {
  const { score, review, critic } = s;
